...
function initStaticFormValidation() {
  ...
  const initSearchableSelects = () => {
    document.querySelectorAll(".local-registration-form select, .aid-form select").forEach((select) => {
      ...
      const getOptions = () => [...select.options].map((option, index) => ({
        index,
        value: option.value,
        label: option.textContent.trim(),
        option
      }));
      const escapeOptionLabel = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;");
      ...
      const renderList = (query = "") => {
        ...
        list.innerHTML = matches.length ? matches.map(
          ({ index, label, option }) => `
